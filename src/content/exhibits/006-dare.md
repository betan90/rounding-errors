---
title: "Three quarters of a billion dollars a year, effect size 0.023"
exhibit: "006"
country: "United States"
status: "curriculum finally replaced; the t-shirts remain iconic"
category: "indistinguishable from nothing"
tags: ["outlived-the-evidence", "education"]
date: 2026-07-25
teaser: "Drug prevention · the biggest program in America vs its own error bars"
ledgerTitle: "The numbers: West and O'Neal 2004"
ledger:
  - label: "Measured effect on drug use (meta-analysis)"
    value: "d = 0.023"
  - label: "95% confidence interval"
    value: "−0.04 to 0.08"
    gap: true
  - label: "Average annual federal-era expenditure"
    value: "~US$750 million"
ledgerNote: "The confidence interval crosses zero. That is statistics for \"we cannot tell whether this did anything at all,\" at three quarters of a billion dollars a year."
verdict:
  - key: "Fraud detected"
    value: "none."
  - key: "Rules broken"
    value: "none. Everyone showed up, taught the classes, and believed."
  - key: "Usefulness of headline metric"
    value: "the program's KPI was attendance, and attendance was excellent."
    long: true
verdictNote: "Scared Straight ran on a number that never existed. DARE ran on something quieter: no number at all, just the powerful feeling that it must be working, sustained for two decades against every evaluation, at nine figures a year. The most expensive statistic on this site is a zero."
sources:
  - name: "West and O'Neal"
    text: "Project D.A.R.E. Outcome Effectiveness Revisited, American Journal of Public Health (2004): d = 0.023, 95% CI −0.04 to 0.08, nonsignificant; \"largest single school-based prevention program in terms of federal expenditures,\" averaging three quarters of a billion dollars a year."
    url: "https://pmc.ncbi.nlm.nih.gov/articles/PMC1448384"
    urlLabel: "pmc.ncbi.nlm.nih.gov"
  - name: "US General Accounting Office"
    text: "Youth Illicit Drug Use Prevention: DARE Long-Term Evaluations (GAO-03-172R, 2003): six evaluations, no statistically significant differences in illicit drug use."
  - name: "Rosenbaum and Hanson"
    text: "Assessing the Effects of School-Based Drug Education: A Six-Year Multilevel Analysis of Project D.A.R.E., Journal of Research in Crime and Delinquency 35(4) (1998): urban and rural students showed some benefit; suburban DARE graduates showed small but significant increases in drug use versus suburban controls, the so-called boomerang effect."
    url: "https://journals.sagepub.com/doi/10.1177/0022427898035004002"
    urlLabel: "journals.sagepub.com"
  - name: "US Surgeon General"
    text: "Youth Violence: A Report of the Surgeon General (2001), Chapter 5, \"Prevention and Intervention,\" confirmed by direct fetch: DARE is placed under the \"Ineffective Primary Prevention Programs\" section heading, meeting the criteria for \"Does Not Work\" despite being the most widely implemented youth drug prevention program in the country, given evaluations that \"consistently show little or no deterrent effects on substance use.\""
    url: "https://www.ncbi.nlm.nih.gov/books/NBK44295/"
    urlLabel: "ncbi.nlm.nih.gov"
  - name: "Priceonomics"
    text: "DARE: The Anti-Drug Program That Never Actually Worked: the 2000 Department of Education evidence audit; the Surgeon General's observation that DARE's popularity persisted despite evaluations consistently showing little or no deterrent effect."
  - name: "Boston University Law Review"
    text: "Truth or DARE note: the 2007 decision to seek a new curriculum."
disclosure: "This entry was drafted with AI (Claude did the research sweeps and first draft). The editorial voice, the opinions, and any surviving errors are mine. Every figure was checked against the linked primary source before publishing. Fairness note kept on purpose: DARE is the rare institution in this museum that eventually acted on its own null results and replaced its curriculum. The exhibit is about the twenty years before that."
correctionNote: "This exhibit was corrected on 2026-07-25. Details on the corrections page."
---

## The incident

DARE, Drug Abuse Resistance Education, launched out of the Los Angeles Police Department in 1983 and became the biggest thing in American drug prevention: uniformed officers in classrooms, workbooks, graduation certificates, and the most successful t-shirt in the history of public policy. By the height of its run it was the country's largest single school-based prevention program by federal expenditure, averaging around three quarters of a billion dollars a year.

It felt unimprovable. Police and children, together, saying no. Nobody involved was insincere, and that is exactly what makes it an exhibit.

## Who lied

Nobody, which is the pattern with the expensive ones. The evaluations just kept coming back empty, and the institutions eventually said so in their own voices. A six-year randomized study led by Dennis Rosenbaum and Gordon Hanson, published in 1998, found DARE had no lasting effect on drug use for most students, except among suburban graduates, who showed small but significant increases in drug use compared to suburban students who never took the class: the so-called boomerang effect. In 2001, the US Surgeon General placed DARE in the category of Ineffective Primary Prevention Programs. In 2003, the General Accounting Office reviewed six long-term evaluations and found no statistically significant differences in illicit drug use between kids who got DARE in fifth or sixth grade and kids who didn't. In 2000, the Department of Education had already audited its drug-education funding and made programs prove effectiveness to qualify. DARE didn't make the cut.

The definitive arithmetic arrived in a 2004 meta-analysis in the *American Journal of Public Health*: across the published evaluations, the overall effect of DARE on drug use was d = 0.023, confidence interval −0.04 to 0.08, nonsignificant. This one isn't harmful, like the last exhibit. It is, to the limits of measurement, nothing at all.

## The fix

To its credit, and unlike most programs on this site, DARE eventually did the Ireland move: it stopped defending the broken thing and adopted a different one. From 2007 it went looking for a new curriculum and ultimately replaced its original lecture-a-cop format with an externally developed, evidence-based program. The brand survived by swapping out everything inside the brand, which is either a redemption story or a very long-running clerical correction, depending on your mood.
